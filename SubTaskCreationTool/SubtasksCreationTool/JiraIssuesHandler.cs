...
namespace SubtasksCreationTool
{
    public class JiraIssuesHandler : IJiraIssuesHandler
    {
        ...
        public async Task CreateSubtasks(IEnumerable<IssueDto> parentIssues)
        {
            foreach (var issueDto in parentIssues)
            {
                ...
                foreach (var typeOfSubtask in typeOfSubtasksToCreate)
                {
                    ...
                    {
                        if (mainTaskSummary.Contains("implement", StringComparison.OrdinalIgnoreCase) ||
                            mainTaskSummary.Contains("[Dev]", StringComparison.OrdinalIgnoreCase) ||
                            mainTaskSummary.Contains("investigat", StringComparison.OrdinalIgnoreCase))
                        {
        ...
        public async Task SendCommentsAboutAddingPU(IEnumerable<Issue> tasksWithoutNeededPu)
        {
            foreach (Issue issue in tasksWithoutNeededPu)
            {
                if (issue.Fields.DevPreliminaryUnits == null || issue.Fields.DevPreliminaryUnits == 0)
                {
                    ...
                    {
                        Console.WriteLine(
                            $"Sending mention to {issue.Fields.ResponsibleDev.Name} about adding Dev PU for {issue.Key}...");
                ...
                {
                    Console.WriteLine(
                        $"Sending mention to {IlyaShalin.Name} about adding Dev PU for {issue.Key}...");
            ...
            foreach (var issue in issues)
            {
                ...
                if (existingDevSubtasks.Any())
                {
                    ...
                    if (sumEstimate != (int) Math.Truncate(issue.Fields.DevPreliminaryUnits.Value * HoursInPu * 3600))
                    {
                        ...
                        {
                            Console.WriteLine(
                                $"Send mention to {issue.Fields.ResponsibleDev.Name} about updating Dev PU or estimates for {issue.Key}...");
                ...
                if (existingQaSubtasks.Any())
                {
                    ...
                    {
                        Console.WriteLine(
                            $"Send mention to {IlyaShalin.Name} about updating QA PU or estimates for {issue.Key}...");
        ...
        private static string GetOriginalEstimateToString(double originalEstimate)
        {
            ...
            double originalEstimateMinutes =
                (int) Math.Truncate((originalEstimateInDouble - originalEstimateHours) * 60);
        ...
        private async Task ResetTheIssueOriginalEstimate(Issue issue)
        {
            ...
            {
                await _jiraClient.UpdateIssueAsync(issue.Key,
                    new Issue
                    {
